...
# Parse/Upload files to dps.report  #
# Post results on Discord           #
# (works with various discord bots) #
# # # # # # # # # # # # # # # # # # #


class App(QtWidgets.QMainWindow):
    def __init__(self, mw_name, mw_x, mw_y, config_dict, version):
        super().__init__()
    ...
    def mw_center(self):
        ...
        x = int((monitor_res.width() / 2) - (self.frameSize().width() / 2))
        y = int((monitor_res.height() / 2) - (self.frameSize().height() / 2))
        self.move(x, y)
...
def main():
    version = 'v2.00'  # current app version
    window_name = 'Upload Tool'
    window_width = 400
    window_height = 600  # ATTENTION: Widgets relative to (0|0)
    config.checkSettings()  # check settings file
    config_dict = config.read_settings()

    """Init UI"""
    app = QtWidgets.QApplication(sys.argv)
    ex = App(window_name, window_width, window_height, config_dict, version)
    sys.exit(app.exec_())
